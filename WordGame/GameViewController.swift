...
class GameViewController: UIViewController, UIGestureRecognizerDelegate, UIScrollViewDelegate {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        if !UserDefaults.standard.bool(forKey: "launchedWordGameBefore") {
            let viewMask = UIView()
    ...
    private func IAP() {
        ...
        alert.addAction(UIAlertAction(title: "Okay", style: UIAlertActionStyle.default) { (action: UIAlertAction!) in
            self.purchase(productId: "com.fooble.boosts")
        })
    ...
    private func takeTilesOffBoardAndSave() {
        for tile in allTiles {
            if tile.myWhereInPlay == .board && !tile.isLockedInPlace {
                myBoard.slots[tile.slotsIndex!].isOccupied = false
...
//                        }
    ...
    @objc private func playFunc(_ button: UIButton) {
        ...
        guard isFirstPlayFunc == true else {isFirstPlayFunc = true;print("guard1");button.setTitle("1", for: .normal);view.addSubview(button); return}
        ...
        findTheString() { (word,isRowWord,tilesInPlay) -> Void in
            if let w = word {
                ...
                guard isReal(word: w.lowercased()) != nil else {wordTiles.removeAll(); print("guard2");button.setTitle("2", for: .normal);view.addSubview(button);return}
                if isReal(word: w.lowercased())! && everythingChecksOutPerpendicular {
                    var isDetachedLetter = false
                    for tile in tilesInPlay {
                        if !wordTiles.contains(tile) {
                           isDetachedLetter = true
                        }
                    }
                    guard isDetachedLetter == false else {
                    wordTiles.removeAll()
                    ...
                    for i in 0..<wordTiles.count {
                        ...
                        delay(bySeconds: 0.2*Double(i)+0.6) {
                            ...
                            if bonusTiles.contains(self.wordTiles[i].mySymbol.rawValue) && !self.wordTiles[i].isLockedInPlace {
    ...
    private func bonusBoost(text: String) {
        
        let bonus = UILabel(frame: CGRect(x: 290*sw/375, y: 22*sh/667, width: 50*sw/375, height: 16*sw/375))
        bonus.font = UIFont(name: "ArialRoundedMTBold", size: 14*fontSizeMultiplier)
        bonus.textAlignment = .left
        bonus.text = text
        view.addSubview(bonus)
        UIView.animate(withDuration: 2.5) {
            bonus.frame.origin = CGPoint(x: 290*self.sw/375, y: 5*self.sh/667)
            bonus.font.withSize(20*self.fontSizeMultiplier)
            bonus.alpha = 0.0
        }
        delay(bySeconds: 2.5) {
    ...
    private func numberOfTilesInPlay() -> Int {
        var count = 0
        for tile in allTiles {
            if tile.myWhereInPlay == .board && !tile.isLockedInPlace {
                count += 1
                            ...
                            if tile.column! == smallestColumn && tile.row! == smallestRow {
                                ...
                                if !isReal2(word: word.lowercased()){
                                    ...
                                    if perpedicularWordHasNewLetter && word.characters.count > 4 && isAtLeastOneNewTile {
                                        ...
                                        self.myTimer = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(GameViewController.rowPerpendicularLongWordBonus), userInfo: nil, repeats: false)
                                    }
                                }
                            }
                            ...
                            if tile.row! == smallestRow && tile.column! == smallestColumn {
                                ...
                                if !isReal2(word: word.lowercased()) {
                                    ...
                                    if perpedicularWordHasNewLetter && word.characters.count > 4 {
                                        ...
                                        self.myTimer = Timer.scheduledTimer(timeInterval: 3.0, target: self, selector: #selector(GameViewController.columnPerpendicularLongWordBonus), userInfo: nil, repeats: false)
                                    }
    ...
    @objc private func rowPerpendicularLongWordBonus() {
        if self.isMainWordReal || self.numberOfTilesInPlay() == 1 {
            self.longWordBonus(length: length)
        }
    }
    
    @objc private func columnPerpendicularLongWordBonus() {
    ...
    func isReal(word: String) -> Bool? {
        ...
        guard isWordBuildable == true else {
            ...
            alert.addAction(UIAlertAction(title: "Okay", style: UIAlertActionStyle.default, handler: nil))
            self.present(alert, animated: true, completion: nil)
            print("guard3");button.setTitle("3", for: .normal);view.addSubview(button)
            return nil
        }
    ...
    @objc private func moveTile2(_ gesture: UIPanGestureRecognizer) {
        ...
        case .began:
            once = true
            for tile in allTiles {
                if tile.frame.contains(gesture.location(in: view)) && (tile.myWhereInPlay == .atBat) {
                    let alert = UIAlertController(title: "", message: "Your rack of letters is no longer full. Fill it with the tiles on the bottom", preferredStyle: UIAlertControllerStyle.alert)
                    alert.addAction(UIAlertAction(title: "Okay", style: UIAlertActionStyle.default, handler: nil))
                    self.present(alert, animated: true, completion: nil)
    ...
    func dropTileWhereItBelongs(tile: Tile) {
        ...
        case .atBat:
            guard let order = tile.atBatTileOrder else {tile.atBatTileOrder = 1; rearrangeAtBat(); return}
            let x = 15*self.sw/375 + CGFloat(order)*49.7*self.sw/375
    ...
    func purchase(productId: String) {
        ...
        SwiftyStoreKit.purchaseProduct(productId) { result in
            ...
            case .success( _):
                ...
                if productId == "com.fooble.boosts" {
                    
                    self.amountOfBoosts += 3
                    self.bonusBoost(text: "+3")
                    self.storeWholeState()
                    LoadSaveCoreData.sharedInstance.saveState()
